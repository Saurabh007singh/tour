...
export async function generateStaticParams() {
  const ids = [
    "beach","mountians","adventure","pilgrim","wildlife"    
  ]; // Add your actual IDs here
  return ids.map((id) => ({ id }));
}

export default async function SpecificCategories({params}){
    const {id}=await params
     const pack = packages.filter((item) => item.category.includes(id));
